...
export default function Sidebar() {
  ...
  return (
    <aside className={`bg-[#1a2234] text-white min-h-screen flex flex-col transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-56'}`}>
      {/* Logo */}
        ...
        <div className="flex flex-col items-center">
          <div className="w-14 h-14 flex items-center justify-center mb-1">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
              ...
              <path d="M2 12 C6 8, 18 16, 22 12" stroke="currentColor" strokeWidth="1" fill="none"/>
            </svg>
          </div>
          {!isCollapsed && (
            <span className="text-[10px] font-medium tracking-[0.2em] text-gray-300">SUCOFINDO</span>
          )}
        </div>
      </div>
          ...
          {!isCollapsed && (
            <>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-white">Ivan</p>
        ...
        <ul className="space-y-0.5 px-2">
          {menuItems.map((item) => (
            <li key={item.label}>
              ...
              {/* Submenu Items */}
              {!isCollapsed && item.hasSubmenu && item.subMenuItems && expandedMenu === item.label && (
